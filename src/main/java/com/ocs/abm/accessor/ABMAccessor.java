...
//	private static String dest_host = "192.168.1.133";
	private static String dest_host = "127.0.0.1";
	private static int dest_port = 3878;
...
//			System.out.println(c.toString());
	...
	private static Message generateAOQMassage(AOQParams params, MessageHeader messageHeader){
		...
		//  < Session-Id >
		msg.add(new AVP_UTF8String(ProtocolConstants.DI_SESSION_ID, params.getSessionID()));
		//  { Origin-Host }
		//  { Origin-Realm }
		ssc.node().addOurHostAndRealm(msg);
		//  { Destination-Realm }
		msg.add(new AVP_UTF8String(ProtocolConstants.DI_DESTINATION_REALM,"cmcc.com"));
		//  [ Destination-Host ]
		msg.add(new AVP_UTF8String(ProtocolConstants.DI_DESTINATION_HOST,dest_host));
				
		//  { CC-Request-Type } 不同的request类型不同了！
		msg.add(new AVP_Unsigned32(ProtocolConstants.DI_CC_REQUEST_TYPE,ProtocolConstants.DI_CC_REQUEST_TYPE_INITIAL_REQUEST));
		//  [ Event-Timestamp ]
		msg.add(new AVP_Time(ProtocolConstants.DI_EVENT_TIMESTAMP,(int)(System.currentTimeMillis()/1000)));
//		//  { Service-Identifier }
//		msg.add(new AVP_Unsigned32(ProtocolConstants.DI_SERVICE_IDENTIFIER,0));
		//  [ Actual-Time ]
...
//					   new AVP_UTF8String(ProtocolConstants.DI_SUBSCRIPTION_ID_DATA, "13430321124")}));
		...
		// { Subscriber-id }
		msg.add(new AVP_UTF8String(ProtocolConstants._3GPP_DI_SUBSCRIBER_ID, params.getSubscriberID())); // ？？
				
		// { Account-id }
		msg.add(new AVP_UTF8String(ProtocolConstants._3GPP_DI_ACCOUNT_ID, params.getAccountID())); // ？？
		...
		deduct_operation_avps.add(new AVP_Unsigned32(ProtocolConstants.DI_SERVICE_IDENTIFIER, 0));
		
		if(params.getDeductBalances().size()>0){
			ArrayList<AVP_Grouped> account_items = new ArrayList<AVP_Grouped>();
			for( DeductBalance db : params.getDeductBalances()){
				AVP_Grouped account_item = new AVP_Grouped(ProtocolConstants._3GPP_DI_ACCOUNT_ITEM,
						new AVP[] { new AVP_Unsigned64(ProtocolConstants._3GPP_DI_ACCOUNT_ITEM_TYPE ,  db.getAccountItemType()),
						new AVP_Float32(ProtocolConstants._3GPP_DI_CHANGE_VALUE, (float)db.getChgValue())});
				account_items.add(account_item);
			}
		
			DeductBalance firstDb = params.getDeductBalances().get(0);
			ArrayList<AVP> items = new ArrayList<AVP>();
			items.add(new AVP_Unsigned64(ProtocolConstants._3GPP_DI_BALANCE_ID, firstDb.getBalanceID()));
			items.add(new AVP_Integer32(ProtocolConstants._3GPP_DI_BALANCE_TYPE, (int)firstDb.getBalanceType()));
			items.addAll(account_items);
			items.add(new AVP_Float32(ProtocolConstants._3GPP_DI_CHANGE_VALUE, 0));
			items.add(new AVP_Unsigned32(ProtocolConstants._3GPP_DI_CLEAR_RESERVE_INDICATOR, firstDb.getClearReserveIndicator()));	
			...
			AVP_Grouped balance = new AVP_Grouped(ProtocolConstants._3GPP_DI_BALANCE, itemsTmp);
			deduct_operation_avps.add(balance);
		}
		
		for( DeductCounter dc : params.getDeductCounters() ){
			AVP_Grouped counter_item = new AVP_Grouped(ProtocolConstants._3GPP_DI_COUNTER,
								new AVP[]{ new AVP_Unsigned64(ProtocolConstants._3GPP_DI_COUNTER_ID, dc.getCounterID()),
						           new AVP_Float32(ProtocolConstants._3GPP_DI_CHANGE_VALUE, (float)dc.getChgValue())
			});
		...
		msg.add(new AVP_Grouped(ProtocolConstants._3GPP_DI_MULTIPLE_DEDUCT_OPERATION, operation_avps));
				...
				//				[ Change-Value ]
				//			{ Change-Value }
				AVP_Grouped account_item_r = new AVP_Grouped(ProtocolConstants._3GPP_DI_ACCOUNT_ITEM,
						new AVP[] { new AVP_Unsigned64(ProtocolConstants._3GPP_DI_ACCOUNT_ITEM_TYPE ,  0),
									new AVP_Float32(ProtocolConstants._3GPP_DI_CHANGE_VALUE, 1)});
				
				AVP_Grouped balance_r = new AVP_Grouped(ProtocolConstants._3GPP_DI_BALANCE,
						new AVP[] { new AVP_Unsigned64(ProtocolConstants._3GPP_DI_BALANCE_ID, 1000),
									new AVP_Integer32(ProtocolConstants._3GPP_DI_BALANCE_TYPE, 0),
									account_item_r,
									new AVP_Float32(ProtocolConstants._3GPP_DI_CHANGE_VALUE, 0)
				});
				msg.add(new AVP_Grouped(ProtocolConstants._3GPP_DI_MULTIPLE_RESERVE_OPERATION,
						new AVP[]{new AVP_Unsigned32(ProtocolConstants.DI_SERVICE_IDENTIFIER, 0),
									balance_r
				}));

				
		msg.add(new AVP_Unsigned32(ProtocolConstants.DI_AUTH_APPLICATION_ID,ProtocolConstants.DIAMETER_APPLICATION_CREDIT_CONTROL)); // a lie but a minor one
		return msg;
	}
	...
	public static AOQResult sendAOQ(AOQParams params) throws EmptyHostNameException, IOException, UnsupportedTransportProtocolException, InterruptedException{
		prepareNode();
				...
				for ( AVP avp : answer.subset(ProtocolConstants._3GPP_DI_COUNTER)){
					...
					for( AVP tmpAvp : avps ){
						...
						case ProtocolConstants._3GPP_DI_COUNTER_TYPE:
							c.setCounterType(new AVP_UTF8String(tmpAvp).queryValue());
							break;
	...
	private static AOQResult processAOQResponse(Message answer) {
...
//				}
				...
				for ( AVP avp : answer.subset(ProtocolConstants._3GPP_DI_MULTIPLE_DEDUCT_OPERATION)){
					AVP[] gavps = new AVP_Grouped(avp).queryAVPs();
					for( AVP a : gavps ){
						if(a.code == ProtocolConstants.DI_RESULT_CODE){
							int v = new AVP_Integer32(a).queryValue();
							deductRslt = deductRslt & (v==ProtocolConstants.DIAMETER_RESULT_SUCCESS);
						}
					}
				}
